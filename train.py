...
frame_skip_rate = 0 
SCALE_FACTOR = 4
face_cascade = cv2.cv2.CascadeClassifier("./MacaqueFrontalFaceModel.xml") 
sideFace_cascade = cv2.cv2.CascadeClassifier("./haarcascade_profileface.xml")
...
while ret:
    ...
    if frame_skip_rate == 0:
        ...
        for rotation in current_rotation_map:
            ...
            gray = cv2.cv2.cvtColor(rotated_frame, cv2.cv2.COLOR_BGR2GRAY)
            ...
            if len(faces):
                for f in faces:
                    ...
                    cropped_face = cv2.cv2.resize(cropped_face, FACE_DIM, interpolation = cv2.cv2.INTER_AREA)
                    cv2.cv2.rectangle(rotated_frame, (x,y), (x+w,y+h), (0,255,0))
                    cv2.cv2.putText(rotated_frame, "Training Face", (x,y), cv2.cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0,255,0))
    ...
    cv2.cv2.putText(processed_frame, "Press ESC or 'q' to quit.", (5, 15), cv2.cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255))

    cv2.cv2.imshow("Real Time Facial Recognition", processed_frame)



    if len(cropped_face):
        cv2.cv2.imshow("Cropped Face", cv2.cv2.cvtColor(cropped_face, cv2.cv2.COLOR_BGR2GRAY))
        if num_of_face_saved < num_of_face_to_collect and key == ord('p'):
            face_to_save = cv2.cv2.resize(cropped_face, (50, 50), interpolation = cv2.cv2.INTER_AREA)
            face_name = profile_folder_path+str(num_of_face_saved)+".png"
            cv2.cv2.imwrite(face_name, face_to_save)
